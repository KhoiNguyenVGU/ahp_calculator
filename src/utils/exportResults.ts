...
export function exportToCSV(
  ...
  const ranked = alternatives
    .map((alt, idx) => ({
      ...
      distBest: result.topsisResult.distanceFromBest[idx],
      distWorst: result.topsisResult.distanceFromWorst[idx],
      index: idx,
    }))
  ...
  alternatives.forEach((alt, i) => {
    ...
    criteria.forEach((_, j) => {
      csv += ',' + result.topsisResult.rawMatrix[i][j].toFixed(2);
    });
    csv += '\n';
  });
  ...
  const exportData = {
    ...
    rankings: alternatives
      .map((alt, idx) => ({
        rank: idx,
        candidate: alt,
        closenessScore: result.topsisResult.performanceScores[idx],
        distanceFromBest: result.topsisResult.distanceFromBest[idx],
        distanceFromWorst: result.topsisResult.distanceFromWorst[idx],
      }))
    ...
    decisionMatrix: alternatives.map((alt, i) => ({
      candidate: alt,
      scores: criteria.map((_, j) => result.topsisResult.rawMatrix[i][j]),
    })),
  };
...
<body>
  <div class="container">
      ...
      <tbody>
        ${alternatives
          ...
          <tr>
            ...
            ${criteria.map((_, j) => `<td>${result.topsisResult.rawMatrix[i][j].toFixed(2)}</td>`).join('')}
          </tr>
        `
          )
          .join('')}
    ...
    <div class="footer">
      ...
      <p>This report was generated using Hybrid Fuzzy AHP + TOPSIS methodology to ensure objective candidate evaluation.</p>
    </div>
  </div>
</body>
</html>
